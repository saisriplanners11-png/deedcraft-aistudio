...
export type ExtractionProfile =
  | 'phase1:linkDoc' | 'phase1:houseTax' | 'phase1:titleDeed' | 'phase1:nala' | 'phase1:permissions' | 'phase1:layoutLrs'
...
export function sectionFields(profile: ExtractionProfile) {
  ...
  const ownDetailsTitle: Record<string, string> = {
    ...
    layoutLrs: 'Layout and LRS details',
  };
...
export function sameTranscription(a: Candidate, b: Candidate) {
  const normalize = (v: string) => {
    ...
    if(/^(nearHNo|bearingHNo)$/.test(a.field))value=value.replace(/^(?:(?:near|adjacent)\s+)?(?:h\.?\s*no\.?|house\s*(?:no\.?|number))\s*[:.-]?\s*/i,'');
...
function profileInstruction(profile: ExtractionProfile) {
  ...
  if (profile === 'jurisdiction') return 'This file was uploaded specifically to fill Jurisdiction for the current property. Return only propState, districtRegistrar, sro, district, mandal, village and ulbAuthority, all with role="property" and record="primary". Return ulbAuthority only when the source explicitly names Municipality, Gram Panchayit, Municipal Corporation or GHMC. Do not return link-document, property-schedule, party, valuation or payment facts.';
...
const LINK_TASK_INSTRUCTIONS = {
  ...
  step3: 'Extract subject PROPERTY jurisdiction, prioritizing the main property schedule over party residence and office letterheads. district is the REVENUE district the property lies in, and districtRegistrar is the Registration District named beside "District Registrar"/"Registration Jurisdiction of District Registrar" — most schedules state only one district name for both, in which case copy that same printed name into both district and districtRegistrar rather than leaving district blank; only give them different values when the schedule visibly prints two distinct district names. Never take district from a party\'s own address block. Copy printed place names without words proper, village, or mandal. propState must be normalized to exactly "Telangana" or "Andhra Pradesh" — the only two allowed values — even when the document prints "Telangana State", "T.S.", "TS" or similar; infer it from the printed district/SRO when the state itself is not spelled out. Set ulbAuthority only when Municipality, Gram Panchayit, Municipal Corporation or GHMC is explicitly printed. Use role=property, record=primary.',
  step4: 'Extract the MAIN subject property and schedule, prioritizing its verbatim schedule over plan headings and neighboring property labels. Copy all four explicitly labeled boundaries, every subject identifier and both alternate extents. A site-plan file number is NOT a building permit number. A neighboring H.No. goes only into nearHNo; when the source explicitly says Near or Adjacent, return that relationship separately in nearAdjacent. Copy the subject property PTIN / P.T.I. No. / Property Tax Identification No. / Property Tax ID / Assessment No. only into bltNo; never use demand, receipt, survey, house-number or amount values. Use the schedule wording for category: part open plot means Part open place. Use role=property, record=primary. Preserve separate records only if the instrument actually conveys multiple distinct scheduled properties.',
};
